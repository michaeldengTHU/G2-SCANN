...
def g2mann(dataset,nclust):
    data0=np.loadtxt('../real_dataset/'+dataset+'_new.txt')[:,:-1]
    ...
    if os.path.exists('../real_dataset/'+dataset+'.gb0'):
        gb_dist = np.loadtxt('../real_dataset/'+dataset+'.gb0')                  # gbd: without de-outlier; gb0: with dynamic natural neighbor
    else:
        ...
        np.savetxt('../real_dataset/'+dataset+'.gb0', gb_dist)
...
if __name__ == '__main__':
    all_datasets=['Iris','WDBC','Seeds','Libras_movement','Ionosphere','Dermatology']
    all_gt_n_clusters=[3,2,3,15,2,6]
    ...
    for i in range(len(datasets)):
        ...
        gt=np.loadtxt('../real_dataset/' + dataset + '_new.txt')[:,-1]
        pred=pred.reshape((-1))
        ari = adjusted_rand_score(gt, pred)
        ami = adjusted_mutual_info_score(gt, pred)
        print(dataset.center(15, ' ')  + ' n_clusters=%d ami=%.4f ari=%.4f\n' % (n_cluster, ami, ari))
        # np.savetxt('newdataset/G2-MANN/'+dataset+'_pred.txt',pred)
    # np.savetxt('newdataset/G2-MANN/amiari.txt',np.array(amiari))